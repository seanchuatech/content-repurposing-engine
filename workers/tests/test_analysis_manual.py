...
async def test_manual_analysis():
    ...
    with patch(
        "src.pipeline.analyze.llm_service.generate_json", new_callable=AsyncMock
    ) as mock_gen:
        ...
        for clip in clips:
            print(
                f" - {clip.title} ({clip.virality_score}%): "
        ...
        if os.path.exists(analysis_path):
            ...
            with open(analysis_path) as f:
                ...
                print(
                    f"Saved data check: {len(saved_data.get('clips', []))} "
                    f"clips found in file."
                )
        else:
