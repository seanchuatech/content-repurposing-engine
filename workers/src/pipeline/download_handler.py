...
async def update_download_status(download_id: str, status: str, progress: int = 0, file_path: str = None, file_name: str = None, file_size: int = None, failed_reason: str = None):
    ...
    if file_size: payload["fileSize"] = file_size
        ...
        if d['status'] == 'downloading':
            ...
            if total and total > 0:
                ...
                if percent >= last_reported_progress + 5:
                    ...
                    # Schedule coroutine execution threadsafe since callback runs in thread
                    asyncio.run_coroutine_threadsafe(
                        update_download_status(payload.downloadId, "DOWNLOADING", percent), 
                        loop
                    )
