...
class FFmpegService:
    ...
    async def extract_segment(self, input_path: str, output_path: str, start_time: float, end_time: float):
        """
        Extracts a segment from a video file without re-encoding (stream copy) for speed.
    ...
    async def burn_subtitles(self, input_path: str, output_path: str, srt_path: str):
        ...
        command = [
            ...
            "-y",
            "-i", input_path,
            "-vf", f"subtitles='{srt_path}':force_style='FontSize=24,PrimaryColour=&H00FFFFFF,OutlineColour=&H00000000,BorderStyle=1,Outline=1,Shadow=0,Alignment=2'",
            "-c:v", "libx264",
            "-preset", "veryfast",
            "-crf", "22",
            "-c:a", "copy",
    ...
    async def reframe_to_916(self, input_path: str, output_path: str):
        ...
        # We also scale the output to a standard 1080x1920 or similar for consistency if needed,
        ...
        command = [
            ...
            "-y",
            "-i", input_path,
            "-vf", "crop=ih*9/16:ih:(iw-ow)/2:0,scale=1080:1920:force_original_aspect_ratio=decrease,pad=1080:1920:(ow-iw)/2:(oh-ih)/2",
            "-c:v", "libx264",
            "-preset", "veryfast",
            "-crf", "22",
            "-c:a", "copy",
