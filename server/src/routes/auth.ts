...
export const authRoutes = new Elysia({ prefix: '/auth' })
  .get(
    '/google',
    async ({
      cookie: { google_oauth_state, google_oauth_code_verifier },
      set,
    }) => {
  ...
  .get(
    ...
    async ({
      ...
      const storedCodeVerifier = google_oauth_code_verifier?.value as
        | string
        | undefined;
      ...
      try {
        ...
        set.redirect = `${clientUrl}/#/login?token=${token}&user=${encodeURIComponent(
          JSON.stringify({
            id: userWithSub.id,
            email: userWithSub.email,
  ...
  .post(
    ...
    async ({ body, set }) => {
      if (!AuthService.isEmailAllowed(body.email)) {
        set.status = 403;
        return {
          error: 'This demo is restricted to allowed email addresses only.',
  ...
  .get(
    ...
    async ({
      ...
      const userWithSub = await AuthService.getUserWithSubscription(
        user.userId,
      );
      if (!userWithSub) {
        set.status = 401;
        return {
          error: 'Unauthorized - No subscription found',
          code: 'UNAUTHORIZED',
        };
      }
